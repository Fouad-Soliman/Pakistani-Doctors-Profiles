...
input_df=pd.read_csv(r"DoctorFeePrediction_classification_test.csv")
predict_df=pd.read_csv(r"TrainColumns.csv")
...
input_df['Fee Category']=input_df['Fee Category'].map(doctors_fee)
y_test=input_df["Fee Category"]
input_df.drop(columns="Fee Category",axis=1,inplace=True)
Preprocess(input_df)
